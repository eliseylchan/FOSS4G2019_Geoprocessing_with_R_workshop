...
We will use the data from the {spData}[^03-geoprocessing-1] package. Let's see what it contains :

[^03-geoprocessing-1]: More details on the datasets here : <https://cran.r-project.org/web/packages/spData/spData.pdf>
...
If we want to represents our data with proportional symbols, we might want to have centroids. {sf} provides two functions in order to do that:
...
As we want to save it to GeoPackage[^03-geoprocessing-2], we also need to provide a layer name : *london_boroughs_27700*. Repeat for all data you want to save.

[^03-geoprocessing-2]: Because [GeoPackage are cool !](http://switchfromshapefile.org/#geopackage)
...
The `layer_options = "OVERWRITE=true"` ensure you can write on existing layer, it is optional.
```
